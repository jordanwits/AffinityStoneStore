...
interface UpdateOrderStatusData {
   ...
   * so correcting a reference number or adding an internal note no longer re-announces a
...
export async function sendPickupNotice(
  ...
  const prof = (order as any)?.profiles;
  const customerEmail = prof?.email?.trim() || '';
...
export async function refundOrder(
  ...
  if (restrictedRefund > 0) {
    ...
    if (rErr) {
      console.error('Error refunding affinity points:', rErr);
