...
export default async function AdminReportsPage() {
  ...
  if (!isDevMode) {
    ...
    const [pointsIssuedResult, pointsRedeemedResult, topProductsResult] = await Promise.all([
      ...
      supabase.from('order_items').select('product_name, total_points').order('total_points', { ascending: false }).limit(10),
    ]);
    ...
    topProducts?.forEach((item) => {
      productSummary[item.product_name] = (productSummary[item.product_name] || 0) + item.total_points;
    });
    ...
    totalRedeemed = 1500;
    sortedProducts = [
      ['Company Logo T-Shirt', 800],
      ['Insulated Water Bottle', 500],
      ['Laptop Backpack', 200],
    ];
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <h2 className="text-lg font-semibold text-gray-900">Most Redeemed Products</h2>
          </CardHeader>
          <CardContent>
            {sortedProducts.length > 0 ? (
              <div className="space-y-3">
                {sortedProducts.map(([name, points], index) => (
              ...
              <p className="text-gray-500 text-center py-8">
                {isDevMode ? 'Mock data shown (configure Supabase to see real data)' : 'No redemptions yet'}
              </p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex justify-between py-2">
                ...
                <span className="text-sm font-semibold text-gray-900">See Users Tab</span>
              </div>
            </div>
            {isDevMode && (
