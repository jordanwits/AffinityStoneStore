...
export function HistoryFilters({ currentDays }: HistoryFiltersProps) {
  ...
  }, [startDateParam, endDateParam, typeParam, minPointsParam, maxPointsParam, reasonParam]);
  ...
  const navigateWithScrollPreservation = (url: string, e?: React.MouseEvent | React.KeyboardEvent) => {
    ...
    if (scrollY > 100) {
      sessionStorage.setItem('pointsHistoryScroll', scrollY.toString());
  ...
  useEffect(() => {
    const savedScroll = sessionStorage.getItem('pointsHistoryScroll');
    ...
    if (savedScroll) {
      ...
      setTimeout(() => {
        sessionStorage.removeItem('pointsHistoryScroll');
        timeouts.forEach(clearTimeout);
      }, 400);
    }
  }, [daysParam, startDateParam, endDateParam, typeParam, minPointsParam, maxPointsParam, reasonParam]);
  ...
  const clearFilters = () => {
    navigateWithScrollPreservation('/points-history?page=1');
  };

  const hasActiveFilters = !!(startDate || endDate || type !== 'all' || minPoints || maxPoints || reason);

  // Determine if we're using custom date range vs quick filter
  const isCustomDateRange = !daysParam && (startDate || endDate);
