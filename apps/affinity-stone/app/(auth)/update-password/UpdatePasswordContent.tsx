  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: updateError } = await supabase.auth.updateUser({
        password: password,
        data: { [MUST_CHANGE_PASSWORD_KEY]: false },
      });
          ...
          <div className="text-center mb-8">
            ...
            {requiredFlow && (
              <Alert variant="info" className="mt-4 text-left">
                <p className="text-sm">
                  Your administrator created your account with a temporary password. For security, you must choose a new password before using the store.
                </p>
              </Alert>
            )}
          </div>
