-- Dual points buckets: universal vs restricted (Affinity collection only)
...
COMMENT ON COLUMN points_ledger.point_type IS 'universal: spend anywhere; restricted: spend only on products in Affinity collection';
...
-- 4. Checkout: spend restricted on Affinity-eligible subtotal first, then universal
...
'Pickup-only: creates order, inventory updates, deducts restricted then universal points for Affinity-eligible lines.';
