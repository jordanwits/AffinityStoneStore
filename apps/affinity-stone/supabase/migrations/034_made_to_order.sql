-- Made-to-order products: orderable regardless of stock, flagged for fulfillment.
--
-- Products marked made_to_order are not kept on hand. They bypass the inventory
-- check at checkout and their variant counts are left untouched (nothing was
-- pulled from a shelf). Each order line records the flag as an immutable
-- snapshot so the fulfillment team still sees it if the product changes later.

ALTER TABLE products
  ADD COLUMN IF NOT EXISTS made_to_order BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN products.made_to_order IS
  'true: not stocked; orderable regardless of inventory and must be procured per order';

ALTER TABLE order_items
  ADD COLUMN IF NOT EXISTS made_to_order BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN order_items.made_to_order IS
  'Snapshot of products.made_to_order when the order was placed';

-- Checkout: same as 031, plus made-to-order handling in both loops
