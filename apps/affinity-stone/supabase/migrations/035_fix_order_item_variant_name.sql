-- Order lines were saving variant_name as NULL, so admins had no size/color to fulfill against.
--
-- The insert guarded the name with `CASE WHEN v_variant IS NOT NULL THEN v_variant.name END`.
-- On a composite value, `IS NOT NULL` is only true when *every* field is non-null, and
-- product_variants has nullable sku / size / color / inventory_count / image_url. Any variant
-- missing one of those tested as NULL and the name was dropped. Track the name in a plain TEXT
-- variable instead. variant_id was always stored correctly, so past orders can be backfilled.
...
-- Backfill the names the old function dropped. Only fills rows that recorded a variant,
-- so lines that genuinely had no variant stay NULL.
UPDATE order_items oi
SET variant_name = pv.name
