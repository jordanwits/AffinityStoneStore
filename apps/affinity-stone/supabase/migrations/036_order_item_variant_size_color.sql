-- Snapshot the variant's size and color on each order line so fulfillment reads the same
-- way regardless of how the product was set up.
--
-- variant_name is a single free-text field whose shape depends on the admin path used:
-- the size x color matrix produces "L - True Navy", while a size-only variant produces
-- just "L". The fulfillment table showed that raw string, so one order could list
-- "L", "One Size - Blue", and "L - True Navy" side by side with no way to tell whether a
-- color was omitted or simply never recorded. Recording size and color as their own
-- columns lets the admin show them as separate fields, and a blank color reads as a
-- genuinely missing value rather than an ambiguous name.
--
-- These are snapshots, matching product_name / variant_name: editing a variant later must
-- not rewrite what a past order was fulfilled against.

ALTER TABLE order_items
  ADD COLUMN IF NOT EXISTS variant_size TEXT,
  ADD COLUMN IF NOT EXISTS variant_color TEXT;

COMMENT ON COLUMN order_items.variant_size IS
  'Snapshot of product_variants.size when the order was placed';

COMMENT ON COLUMN order_items.variant_color IS
  'Snapshot of product_variants.color when the order was placed';

-- Checkout: same as 035, plus the two new snapshot columns
...
BEGIN
  ...
  IF p_delivery_method IS DISTINCT FROM 'pickup' THEN
    RAISE EXCEPTION 'Orders are pickup only; invalid fulfillment method';
  END IF;
  ...
  LOOP
    ...
    IF v_item->>'variant_id' IS NOT NULL THEN
      ...
      -- Made-to-order products are procured per order, so stock never gates them
    ...
    v_affinity_eligible := v_product.collections IS NOT NULL
      AND 'Affinity' = ANY(v_product.collections);

    IF v_affinity_eligible THEN
  ...
  IF v_universal_balance < v_universal_spend THEN
    RAISE EXCEPTION 'Insufficient points. Need % universal points for this cart (you have %). Total order % pts; Affinity-eligible portion % pts.',
  ...
  LOOP
    ...
    IF v_item->>'variant_id' IS NOT NULL THEN
      ...
      -- Nothing came off the shelf for a made-to-order line, so leave the count alone
      IF NOT v_product.made_to_order AND v_variant.inventory_count IS NOT NULL THEN
...
'Pickup-only: creates order, inventory updates (skipped for made-to-order), deducts restricted then universal points for Affinity-eligible lines.';

-- Backfill past orders from the variant they recorded. This is the closest thing to a
-- snapshot available after the fact; a variant edited since the order was placed will
-- backfill with its current values, which is still better than a blank column.
UPDATE order_items oi
SET variant_name  = COALESCE(oi.variant_name, pv.name),
    variant_size  = pv.size,
    variant_color = pv.color
FROM product_variants pv
WHERE oi.variant_id = pv.id
  AND (oi.variant_name IS NULL OR oi.variant_size IS NULL OR oi.variant_color IS NULL);
