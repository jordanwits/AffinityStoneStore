...
--
-- 034 had these products bypass inventory entirely: no stock check, no decrement. That
-- reads correctly for something nobody keeps on a shelf, but the store does hold a few
-- units of some made-to-order products, and those units were being sold over and over
-- while the count sat unchanged. Checkout now takes whatever is on hand (never past zero)
-- and treats the rest of the line as the part to procure. Stock still never gates a
...
-- Checkout: same as 036, plus the partial draw for made-to-order lines
...
'Pickup-only: creates order, draws each line from stock (made-to-order lines take only what is on hand), deducts restricted then universal points for Affinity-eligible lines.';
