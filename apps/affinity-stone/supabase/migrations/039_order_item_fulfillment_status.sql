...
-- Migrations here are pasted into the dashboard SQL Editor by hand (see SETUP.txt), so this
-- file has to survive being run twice. The backfill is the dangerous half: on a second run
-- it would re-derive every line from the order-level status and wipe out whatever staff had
-- marked since. Gating on "did the column already exist" keeps it strictly first-run-only.
DO $$
DECLARE
